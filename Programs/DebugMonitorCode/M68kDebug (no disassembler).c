...
{
    //given by the debugger program, not part of our code
    ...
    while (1) {
        printf("Please enter mode (1 for byte, 2 for word, 3 for long-word): ");
        c = toupper(_getch());
        if (c == '1') {
            size = 1;
            break;
            ...
            printf("You input is not a valid mode.\r\n");
        }
    }
    
    // prompt user for pattern
    while (1) {
        printf("\r\nPlease enter pattern (0, 5, A, F): ");
        c = toupper(_getch());
        if (c == '0') {
            pattern = 0x00;
            break;
            ...
            printf("You input is not a valid pattern.\r\n");
        }
    }
    ...
    while (1) {
        ...
        start_address = (unsigned) Get8HexDigits(0);
        if ((start_address >= 0xf0000000 && start_address <= 0xf0040000)
            || (start_address >= 0x09000000 && start_address <= 0x09800000)) {
            break;
        } else {
            printf("\r\nYour input is not a valid start address\n");
        }
    }
    ...
    while (1) {
        ...
        end_address = (unsigned) Get8HexDigits(0);

        if ((end_address >= 0xf0000000 && end_address <= 0xf0040000 && end_address > start_address) 
            || (end_address >= 0x09000000 && end_address <= 0x09800000 && end_address >= start_address) ) {
            break;
        } else {
            printf("\r\nYour input is not a valid end address\n");
        }
    }

    // check for address alignment
    if ((end_address - start_address) % 4 != 0) {
        printf("\r\nAddress not aligned.\n");
        return;
    }
    ...
    counter = 0;
    for (addr = start_address; addr < end_address; addr += size) {
        if (size == 1) {
            byte_ptr = addr;
            *byte_ptr = pattern;
        } else if (size == 2) {
            word_ptr = addr;
            *word_ptr = (pattern << 8) | pattern;
        } else if (size == 4) {
            long_ptr = addr;
            *long_ptr = (pattern << 24) | (pattern << 16) | (pattern << 8) | pattern;
        }

        if (counter == 10000) {
            counter = 0;
            printf("Write up to %08x\n", addr);
        }
        counter++;
    }

    // reading and checking the memory, display error message if needed
    counter = 0;
    for (addr = start_address; addr < end_address; addr += size) {
        
        if (size == 1) {
            byte_ptr = addr;
            if (*byte_ptr != (char) pattern) {
                printf("ERROR: address %08x, write value %02x, read value %02x\r\n",
                    addr, pattern, *byte_ptr);
            }
        } else if (size == 2) {
            word_ptr = addr;
            if (*word_ptr != (unsigned short) ((pattern << 8) | pattern)) {
                printf("ERROR: address %08x, write value %04x, read value %04x\r\n",
                    addr, (pattern << 8) | pattern, *word_ptr);
            }
        } else if (size == 4) {
            long_ptr = addr;            
            if (*long_ptr != (unsigned long) ((pattern << 24) | (pattern << 16) | (pattern << 8) | pattern)) {
                printf("ERROR: address %08x, write value %08x, read value %08x\r\n",
                    addr, (pattern << 24) | (pattern << 16) | (pattern << 8) | pattern, *long_ptr);
            };
        }

        if (counter == 10000) {
            counter = 0;
            printf("Read up to %08x\n", addr);
        }
        counter++;
    }

    // the test has finished
    printf("Memory Test completes\r\n");
}
...
// Initialise Breakpoint variables
    ...
    printf("\nSam (91469478) and Sandy (71623268)");
