...
void ProgramFlashChip(void)
{
    ...
    //
    unsigned char *ramPtr = ProgramStart;
    unsigned int i = 0, j = 0, addr=0;
    unsigned char rBuf[256];
    unsigned char wBuf[256];
    int PassFailFlag = 1;
    
    printf("\r\nErasing SPI Flash Chip before flashing...");
    EraseSPIChip();
    ...
    printf("\r\nDone writing.");
    // Reset for verifiying
    addr = 0;
    ramPtr = ProgramStart;
    printf("\r\nReading and checking entire 256K bytes: ");
    for (i = 0; i < 1024; i++){
        ReadSPIData(addr, rBuf, 256);
        addr += 256;
        for (j = 0; j < 256; j++){
            if (ramPtr[j] != rBuf[j]){
                printf("\r\nERR: Mismatch at addr 0x%08x. Wrote: 0x%02x Received: 0x%02x\r\n", &ramPtr[j], ramPtr[j], rBuf[j]);
                PassFailFlag = 0;
            }
        }
        ramPtr+=256;
...
void MemoryTest(void)
{
    ...
    while(1)    {
        ...
        if( c == (char)('B')){
            ...
            bwl = 0x2;
            word = (short)(data | data << 8);

            break;
        }
        else if( c == (char)('L')){
            bwl = 0x3;
            longword = data | data << 8 | data << 16 | data << 24;
            break;
        }   
    }
    ...
    RamPtrLongword = Start;
    RamPtrWord = Start;
    RamPtrByte = Start;
    // Write loop
    while(1){
        // When end addr is reached
        if (RamPtrLongword > End || RamPtrByte > End ||RamPtrWord > End){
            printf("\r\nWrite complete. Starting read.");
            break;
        }
        // Set data at address to specified by user
        
        if(bwl == (unsigned int)(0x3)) *RamPtrLongword = longword;
        else if(bwl == (unsigned int)(0x2)) *RamPtrWord = word;
        else *RamPtrByte = byte;
        
        counter1++;
        // Dont check every time, just check some time incl first time
        if (counter1 == 0x8cc){
            if(bwl == (unsigned int)(0x3)) printf("\r\nWrite: 0x%x to addr 0x%x", longword, RamPtrLongword);
            else if(bwl == (unsigned int)(0x2)) printf("\r\nWrite: 0x%x to addr 0x%x", word, RamPtrWord);
            else if(bwl == (unsigned int)(0x1)) printf("\r\nWrite: 0x%x to addr 0x%x", byte, RamPtrByte);
            counter1 = 1;
        }
        // Increment address
        RamPtrLongword++;
    ...
    while(1){
        ...
        if(bwl == (unsigned int)(0x3)) if (*RamPtrLongword != longword) err = 1;
        else if(bwl == (unsigned int)(0x2)) if (*RamPtrWord != word) err = 1;
        else if(bwl == (unsigned int)(0x1)) if (*RamPtrByte != byte) err = 1;
        if (err){
            if(bwl == (unsigned int)(0x3)) printf("\r\ERROR: Address 0x%x data is 0x%x but should be 0x%x", RamPtrLongword, *RamPtrLongword, longword);
            else if(bwl == (unsigned int)(0x2)) printf("\r\ERROR: Address 0x%x data is 0x%x but should be 0x%x", RamPtrWord, *RamPtrWord, word);
            else if(bwl == (unsigned int)(0x1)) printf("\r\ERROR: Address 0x%x data is 0x%x but should be 0x%x", RamPtrByte, *RamPtrByte, byte);
        ...
        // Dont check every time, just check some time incl first time
        if (counter1 == 0x8cc){
            if(bwl == (unsigned int)(0x3)) printf("\r\nRead: Address 0x%x data is 0x%x", RamPtrLongword, *RamPtrLongword);
            else if(bwl == (unsigned int)(0x2)) printf("\r\nRead: Address 0x%x data is 0x%x", RamPtrWord, (short)*RamPtrWord);
            else if(bwl == (unsigned int)(0x1)) printf("\r\nRead: Address 0x%x data is 0x%x", RamPtrByte, (char)*RamPtrByte);
            counter1 = 1;
        }
        // Increment address
        RamPtrLongword++;
...
void main(void)
{
    ...
    char *BugMessage = "Lab 3";
    char *CopyrightMessage = "Martin Chua 35713411, Varun Rana 21503164";
