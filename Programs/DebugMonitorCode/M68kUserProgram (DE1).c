...
*************************************************************************************************/
    ...
    printf("\n");
    while (1) {
        printf("Please enter mode (byte, word, long-word): ");
        scanf("%s", text);
        if (strcmp(text, "byte") == 0) {
            ...
            size = 2;
            break;
        } else if (strcmp(text, "long-word") == 0) {
            size = 4;
            break;
        } else {
    ...
    while (1) {
        printf("Please enter pattern (00, 55, AA, FF): ");
        scanf("%2s", text);
        if (strcmp(text, "00") == 0) {
            pattern = 0x00;
            break;
        } else if (strcmp(text, "55") == 0) {
            pattern = 0x55;
            break;
        } else if (strcmp(text, "AA") == 0) {
            pattern = 0xAA;
            break;
        } else if (strcmp(text, "FF") == 0) {
            pattern = 0xFF;
            break;
        } else {
    ...
    while (1) {
        ...
        scanf("%8x", &start_address);
        if (start_address < 0x08020000 || start_address > 0x08030000) {
            printf("Your input is not a valid start address\n");
        } else {
            break;
        }
    }
    ...
    while (1) {
        printf("Please enter end address: ");
        scanf("%8x", &end_address);

        if (end_address < 0x08020000 || end_address > 0x08030000 || end_address < start_address) {
            printf("your input is not a valid end address\n");
        } else {
            break;
        }
    }

    if ((end_address - start_address) % size != 0) {
    ...
    for (addr = start_address; addr <= end_address; addr += size) {
        ...
        if((addr - start_address) % (10000 * size) == 0){
            printf("Writes up to %8x\n", addr);
        }
    }
    ...
    for (addr = start_address; addr <= end_address; addr += size) {
        ...
        if (size == 1) {
            byte_ptr = addr;
            if (*byte_ptr != pattern) {
                printf("ERROR: address %8x, write value %2x, read value %2x\n",
                    addr, pattern, *byte_ptr);
            }
        } else if (size == 2) {
            word_ptr = addr;
            *word_ptr = (pattern << 8) | pattern;
            if (*word_ptr != ((pattern << 8) | pattern)) {
                printf("ERROR: address %8x, write value %4x, read value %4x\n",
                    addr, (pattern << 8) | pattern, *word_ptr);
            }
        } else if (size == 4) {
            long_ptr = addr;
            *long_ptr = (pattern << 24) | (pattern << 16) | (pattern << 8) | pattern; 
            if (*long_ptr != ((pattern << 24) | (pattern << 16) | (pattern << 8) | pattern)) {
                printf("ERROR: address %8x, write value %8x, read value %8x\n",
                    addr, (pattern << 24) | (pattern << 16) | (pattern << 8) | pattern, *long_ptr);
            };
        }

        if((addr - start_address) % (10000 * size) == 0){
            printf("Reads up to %8x\n", addr);
        }
    }
